...
const ScreenerFilter = ({ filters, setFilters }) => {
    ...
    return (
        <div className="card" style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: '18px' }}>
            <div>
                <h3 style={{ fontSize: '0.95rem', fontWeight: 700, color: 'var(--text-primary)', marginBottom: '3px', display: 'flex', alignItems: 'center', gap: '6px' }}><Icons.Filter size={16} /> Filter Screener</h3>
                <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>Pilih sinyal & kriteria teknikal</p>
            </div>

            {/* Trading Signals */}
            <div>
                <div style={{ fontSize: '0.68rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: 'var(--text-muted)', marginBottom: '10px', paddingBottom: '7px', borderBottom: '1px solid var(--border-color)', display: 'flex', alignItems: 'center', gap: '4px' }}>
                    <Icons.Activity size={14} /> Sinyal Trading
                </div>
                <CustomSelect name="signal" value={filters.signal || ''} onChange={handleSelect} className="form-select" style={{ fontSize: '0.85rem' }}>
            ...
            <div>
                <div style={{ fontSize: '0.68rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: 'var(--text-muted)', marginBottom: '10px', paddingBottom: '7px', borderBottom: '1px solid var(--border-color)', display: 'flex', alignItems: 'center', gap: '4px' }}>
                    <Icons.BarChart2 size={14} /> Indikator Teknikal
                </div>
            ...
            <div>
                <div style={{ fontSize: '0.68rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: 'var(--text-muted)', marginBottom: '10px', paddingBottom: '7px', borderBottom: '1px solid var(--border-color)', display: 'flex', alignItems: 'center', gap: '4px' }}>
                    <Icons.TrendingUp size={14} /> Pola Chart
                </div>
                <CustomSelect name="pattern" value={filters.pattern || ''} onChange={handleSelect} className="form-select" style={{ fontSize: '0.85rem' }}>
                    ...
                    <option value="candle_bullish">Bullish Candlestick</option>
                </CustomSelect>
            </div>
            ...
            <div>
                <div style={{ fontSize: '0.68rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: 'var(--text-muted)', marginBottom: '10px', paddingBottom: '7px', borderBottom: '1px solid var(--border-color)', display: 'flex', alignItems: 'center', gap: '4px' }}>
                    <Icons.Newspaper size={14} /> Sentimen Berita
                </div>
                <CustomSelect name="sentiment" value={filters.sentiment || ''} onChange={handleSelect} className="form-select" style={{ fontSize: '0.85rem' }}>
            ...
            >
                <Icons.RotateCcw size={14} style={{ marginRight: '6px' }} /> Reset Semua Filter
            </button>
        </div>
    );
};
