...
export default function ReportsPage() {
  ...
  useEffect(() => {
    ...
    async function loadShares() {
      if (!sharingAvailable || !profile?.id) {
        setShareRows([]);
  ...
  }, [profile?.id, sharingAvailable, showToast]);
  ...
  return (
    <div>
      <ReadOnlyNotice
        ...
        description="Halaman ini hanya membuat snapshot read-only. Data jurnal asli tetap mengikuti izin role yang diatur backend."
      />
        ...
        <div className="card-body">
          ...
          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
            <button
              ...
              disabled={!sharingAvailable || shareSaving}
            >
          ...
          {!sharingAvailable ? (
            <div style={{ marginTop: 12, fontSize: '0.82rem', color: 'var(--accent-yellow)' }}>
              Share link publik butuh backend API aktif. Preview report tetap bisa dipakai secara lokal.
            </div>
          ) : null}
        </div>
      </div>
