...
export default function DividendPage() {
  ...
  const [editingId, setEditingId] = useState<string | null>(() => dividendFormDraft?.editingId || null);
  ...
  const handleEdit = (dividend: any) => {
    ...
    setShowForm(true);
    setActiveTab(dividend.market || 'ID');
    setForm({
      stockCode: dividend.stockCode || '',
      shareCount: String(dividend.shareCount ?? ''),
        ...
        <div className="card" style={{ marginBottom: 24, animation: 'fadeInUp 0.3s ease' }}>
          <div className="card-body">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <h3 style={{ margin: 0 }}>
                {editingId ? 'Edit Dividen' : `Catat Dividen ${isUS ? 'Pasar US' : 'Pasar ID'}`}
              </h3>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <button type="submit" className="btn btn-primary">
                <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  <Save size={16} />
                  {editingId ? 'Update Catatan Dividen' : 'Simpan Catatan Dividen'}
                </span>
              </button>
            </form>
          </div>
                  ...
                  <tr key={div.id}>
                    ...
                    <td>
                      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                        <button className="btn btn-ghost btn-sm" onClick={() => handleEdit(div)} aria-label="Edit rekam dividen">
                          <Pencil size={14} />
                        </button>
