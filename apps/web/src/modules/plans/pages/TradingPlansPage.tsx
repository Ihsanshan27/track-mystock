...
export default function TradingPlansPage() {
  ...
  const DRAFT_EDIT_KEY = 'trading_plan_form_edit_id';
  const defaultForm = {
    stockCode: '',
    market: 'ID',
    entryPrice: '',
  ...
  const [editingId, setEditingIdState] = useState<string | null>(
    () => sessionStorage.getItem(DRAFT_EDIT_KEY) || null
  );
  ...
  const setEditingId = (value: string | null) => {
    ...
    if (value) {
      sessionStorage.setItem(DRAFT_EDIT_KEY, value);
      return;
    }
    sessionStorage.removeItem(DRAFT_EDIT_KEY);
  };
  ...
  const activePort = portfolios.find(p => p.id === form.portfolioId) || portfolios[0] || { id: defaultPortfolioId, name: 'Utama' };

  // Calculate Buying Power for risk calculations
  const buyingPower = useMemo(() => {
    const pTrades = trades.filter((t: any) => (t.portfolioId || defaultPortfolioId) === activePort.id);
    const pCashflows = cashflows.filter((c: any) => (c.portfolioId || defaultPortfolioId) === activePort.id);
    const pDividends = dividends.filter((d: any) => (d.portfolioId || defaultPortfolioId) === activePort.id);

    const isUS = form.market === 'US';
    const initialCap = activePort.id === defaultPortfolioId ? (isUS ? (settings.initialCapitalUS ?? 1000) : (settings.initialCapital ?? 10000000)) : 0;
  ...
  }, [trades, cashflows, dividends, activePort.id, defaultPortfolioId, form.market, settings]);
  ...
  const handleEdit = (plan: any) => {
    const nextForm = {
      ...
      targetProfit: String(plan.targetProfit ?? ''),
      riskPercent: String(plan.riskPercent ?? (settings.defaultRiskPercent || 2)),
    ...
    <div>
      <div className="page-header">
        ...
        {canWrite && (
          ...
          >
            ...
            {showForm ? 'Tutup Form' : 'Rencana Baru'}
          </button>
        )}
      </div>
      ...
      {showForm && (
        <div className="card" style={{ marginBottom: 24 }}>
          <div className="card-header"><h3 className="card-title">{editingId ? 'Edit Rencana Trading' : 'Buat Rencana Baru'}</h3></div>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                <button type="submit" className="btn btn-primary">
                  {editingId ? 'Simpan Perubahan' : 'Simpan Rencana'}
                </button>
