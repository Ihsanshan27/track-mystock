...
export default function SettingsPage() {
  ...
  useEffect(() => {
    ...
    async function loadSharingData() {
      if (!can('share:journal') || !user?.id || !sharingAvailable) {
        setAllProfiles([]);
        setSharedAccessRows([]);
  ...
  }, [can, sharingAvailable, showToast, user?.id]);
  ...
  const handleImport = (e) => {
    ...
    reader.onload = async (ev) => {
      try {
        ...
        showToast(error?.message || 'Import data gagal', 'error');
      }
    };
              ...
              <SectionCard title="Share Jurnal ke Mentor / Viewer" description="Atur siapa yang bisa melihat atau mereview jurnal Anda.">
                {!sharingAvailable ? (
                  <div style={{ color: 'var(--accent-yellow)' }}>
                    Fitur sharing jurnal butuh backend API aktif.
                  </div>
                ) : (
                  <>
                    <div className="form-row">
