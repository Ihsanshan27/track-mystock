...
export default function Header({ pageTitle, onMenuToggle }) {
  ...
  const getPortfolioTotalAsset = (portfolioId) => {
    const portfolioTrades = allTrades.filter((item: any) => (item.portfolioId || defaultPortfolioId) === portfolioId);
    const portfolioCashflows = allCashflows.filter((item: any) => (item.portfolioId || defaultPortfolioId) === portfolioId);
    const portfolioDividends = allDividends.filter((item: any) => (item.portfolioId || defaultPortfolioId) === portfolioId);
